...
int main(int argc, char **argv) {
    ...
    float i_hit_rate = ((float)i_hits / (float)(i_hits + i_misses));
    float i_miss_rate = ((float)i_misses / (float)(i_hits + i_misses));

    float d_hit_rate = ((float)d_hits / (float)(d_hits + d_misses));
    float d_miss_rate = ((float)d_misses / (float)(d_hits + d_misses));

    float total_missrate = (float)(i_misses + d_misses) / (float)(i_misses + d_misses + i_hits + d_hits);

    float AMAT = (float)(1.0 + (total_missrate * 100.0));
    ...
    // printf("AMAT: %f\n", AMAT);
    printf("{ \"SET_ASSOCIATIVITY\": %d, \"INSTRUCTION_CACHE_HITS\": %d, \"INSTRUCTION_CACHE_HIT_RATE\": %.2f, \"INSTRUCTION_CACHE_MISSES\": %d, \"INSTRUCTION_CACHE_MISS_RATE\": %.2f, \"DATA_CACHE_HITS\": %d, \"DATA_CACHE_HIT_RATE\": %.2f, \"DATA_CACHE_MISSES\": %d, \"DATA_CACHE_MISS_RATE\": %.2f, \"AMAT\": %.2f }\n",
           set_assoc, i_hits, i_hit_rate, i_misses, i_miss_rate, d_hits, d_hit_rate, d_misses, d_miss_rate, AMAT);
